    ...
    <div style={{ display: "flex", height: "100vh", backgroundColor: theme.bg, color: theme.textMain, overflow: "hidden", fontFamily: "sans-serif" }}>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <header style={{ height: "64px", borderBottom: `1px solid ${theme.border}`, padding: "0 32px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <a href="/auto" style={{ textDecoration: "none" }}>
            <button style={{ padding: "8px 20px", background: theme.accent, color: "white", border: "none", borderRadius: "8px", fontWeight: "bold", cursor: "pointer", display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "32px" }}>
              ...
              <div style={{ backgroundColor: theme.card, borderRadius: "16px", border: `1px solid ${theme.border}`, height: "480px", display: "flex", flexDirection: "column", overflow: "hidden" }}>
                <div style={{ padding: "16px 24px", borderBottom: `1px solid ${theme.border}`, fontWeight: "bold", fontSize: "11px", letterSpacing: "1px" }}>LIVE EXECUTION STREAM</div>
                <div style={{ flex: 1, padding: "24px", overflowY: "auto", fontFamily: "'Fira Code', monospace", backgroundColor: "rgba(0,0,0,0.2)" }}>
                  {logs.map((line, i) => <LogLine key={i} text={line} />)}
                </div>
              </div>

              {/* Target Persistence List */}
              <div style={{ backgroundColor: theme.card, borderRadius: "16px", border: `1px solid ${theme.border}`, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                <div style={{ padding: "16px 24px", borderBottom: `1px solid ${theme.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontWeight: "bold", fontSize: "11px", letterSpacing: "1px" }}>TARGET LIST</span>
                  <Trash2 
                    size={16} 
                ...
                <div style={{ flex: 1, overflowY: "auto" }}>
                  {targets.length === 0 ? (
                    <div style={{ padding: "40px", textAlign: "center", color: "#475569", fontSize: "12px" }}>No active targets.</div>
                  ) : (
                    targets.map((t, i) => (
...
// Updated NavItem to handle actual URL redirection
function NavItem({ icon, label, active, url }) {
  return (
...
function LogLine({ text }) {
  ...
  return <p style={{ color: getColor(text), margin: "0 0 6px", fontSize: "12px", borderLeft: `2px solid ${getColor(text)}`, paddingLeft: "10px" }}>{text}</p>;
}